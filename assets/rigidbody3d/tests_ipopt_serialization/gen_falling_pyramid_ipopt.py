...
def create_xml(size):
    ...
    planes_xml = \
        f"""
<static_plane x="{(s2*(size-1)+1.01)*r} 0 0" n="-1 0 0"/>
...
"""
    ...
    result = \
        f"""
        <rigidbody3d_scene>
  <camera_perspective theta="-1.5707" phi="1.5707" rho="{5+0.2*size}" lookat="0 0 0" up="0 1 0" fps="50" render_at_fps="0" locked="0"/>
  ...
  <impact_operator type="grr" CoR="1.0">
    ...
    os.makedirs('falling_pyramid_ipopt', exist_ok=True)
    with open(os.path.join('falling_pyramid_ipopt', f'{size}.xml'), 'w') as f:
        f.write(result)
